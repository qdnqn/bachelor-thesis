...
ansible --version
\end{lstlisting}
trebali bi dobiti sličan ispis na terminalu:
\begin{lstlisting}
ansible 2.10.2
...
touch Vagrantfile
\end{lstlisting}
U Vagrantfile-u definišemo naše virtualne mašine. Ispod je dat primjer konfiguracije virtualnih mašina korištenih u demonstraciji mogućnosti u ovom radu u sklopu demo direktorija u repozitoriju.
\begin{lstlisting}
...
\\\\
Kada pokrenemo, unutar direktorija \textbf{vagrant/demo},
\begin{lstlisting}
vagrant up
\end{lstlisting}
...
Da bi kreairali bilo kakvu vrstu automatizacije korištenjema Ansible alata potrebno je definisati za koje host-ove je zadužen Ansible. Inventories nam pružaju mogućnost definisanja upravljivih mašina. Ansible inventories se mogu dostaviti u više formata; mogu biti statičke ili dinamičke, i mogu sadržavati bitne varijable na osnovu kojih Ansible pravi odluke pri interakciji sa host-ovima. \\\\
Inventory se sastoji od: hostova, varijabli i grupa. Host predstavlja domenu ili ip adresu mašine kojom želimo upravljati. Grupe omogućavaju da host-ove grupiramo po nekim osobinama ili ulogama. Varijable možemo definisati na nivou host-a ili grupe. Ako dodijelimo varijablu na nivou grupe svi host-ovi unutar te grupe mogu koristiti definisanu varijablu.
...
Ansible zadano koristi inventory koji se nalazi u \textbf{/etc/ansible/hosts}. Iako možemo koristiti grupe unutar inventory file-a preporučljivo je kreirati inventory za svaki projekat zasebno. \\\\ 
Da bi naredili Ansible-u da koristi specifični inventory koristimo sljedeći način.
\begin{lstlisting}
ansible -i InventoryName.yaml all -m ping
\end{lstlisting}
Sa -i zastavicom specificirali smo da ansible koristi inventory InventoryName.yaml. Parametar all govori da se modul ping izvrši nad svim host-ovima. Moguće je koristi ime grupe umjesto \textbf{all}, da ciljamo određenu grupu host-ova. \\\\
Inventory se sastoji od imena host-ova, grupa i varijabli. Zadane grupe koje postoje su all i ungrouped.
Definicija inventory.yaml data je ispod.
\begin{lstlisting}
all:
  hosts:
    u1:
...
\end{lstlisting}
Napomena! Vagrant zadano kreira par javni/privatni ključ pri kreiranju mašine. Zadani user \textbf{vagrant} kojeg koristimo pri SSH konekciji. Da bi vidjeli SSH postavke koje Vagrant koristi potrebno je da u direktoriju, gdje se nalazi Vagrantfile, pokrenemo vagrant ssh-config komandu. Ispis će nam prikazati detaljne informacije o SSH konfiguraciji za svaku virtualnu mašinu. Između ostalog i putanju na host mašini do privatnog ključa potrebnog za autentifikaciju pri SSH konekciji. \textbf{Vrijednost privatnog ključa je potrebno upisati u ansible\_ssh\_private\_key\_file. (Unutar ansible/demo/host\_vars/\*)} Nakon toga ansible može pristupiti mašinama.\\\\
Pokretanjem sljedeće komande možemo provjeriti da li smo dobro definisali inventory (Unutar ansible/demo).
\begin{lstlisting}
...
Iako je moguće u jednom inventory file-u konfigurisati sve host-ove kojima želimo upravljati, na većim projektima poželjno je razdvojiti inventory koristeći strukturu direktorija. \\ Preporučena struktura direktorija data je ispod. \\\\
{\footnotesize
\begin{forest}
  for tree={
    font=\ttfamily,
...
ansible_user: vagrant
\end{lstlisting}
Sadržaj u1.yaml i u2.yaml je dat ispod (\textbf{Potrebno prilagoditi okruženju na kojem se nalazite}).
\begin{lstlisting}
ansible_host: 192.168.33.10
ansible_ssh_private_key_file: /home/qdnqn/vagrant/.vagrant/machines/u1/virtualbox/private_key
\end{lstlisting}
...
\begin{figure}[H]
\centering
 \includegraphics[scale=0.81]{slike/add_aws_user.png}
 \caption{Dodavanje novog korisnika sa programmatic pristupom na AWS platformi.}
 \label{ref_slika2}
\end{figure}

\begin{figure}[H]
\centering
 \includegraphics[scale=0.81]{slike/access_keys.png}
...
\end{lstlisting}
Na slikama ispod je prikazan EC2 dashboard u dva slučaja.
\begin{figure}[H]
\centering
...
\centering
 ...
 \label{ref_slika2}
\end{figure}
Vidimo da je inventory automatski ažuriran bez ikakve intervencije nad inventory-ijem. Prednosti koje nudi ovakav pristup u okruženju gdje se instance rađaju i umiru periodično je značajan.\\\\
Ansible pruža i mogućnost korištenja kombinovanog inventory-ija.
...
\subsubsection{Playbook definicija}
...
\subsubsection{Varijable i petlje}
...
- name: Add new user playbook
  ...
  tasks:
    - name: Add new user task
      ...
      loop: "{{ usernames }}"
\end{lstlisting}
\subsubsection{Uslovno izvršavanje}
...
\subsubsection{Roles}
Jedan od glavnih načina pri izvozu playbook-a je korištenjem \textbf{role}-a. Ispod je definisana i data preporučena standardna struktura Ansible projekta. \\\\
{\footnotesize
\begin{forest}
  for tree={
    font=\ttfamily,
...
\\
Unutar svakog direktorija u role1 nalazi se main.yaml file, ako je taj direktorij potreban za određeni role. U većini slučajeva nije vam potrebno svih 7 direktorija. Međutim one koje koristite moramo je kreirati. 
\\\\
Da bi uvezli role u neki playbook definisat ćemo roles element na sljedeći način.
\begin{lstlisting}
---
- hosts: all
  roles:
...
Jinja2 je široko korišten eng. template engine koji je napisan u Python-u. Ansible nudi mogućnost integracije Jinja2 mogućnosti unutar Ansible-a. \\\\
Definicija jinja2 predložaka se stavlja unutar file-ova koji se nalaze u templates direktoriju unutar Ansible projekta. \\\\
Primjer je dat ispod (ansible/demo/templates/example.j2).
\begin{lstlisting}
Hello world!
Currently I am runnning on virtual machine with the IP: {{ ansible_host }}
\end{lstlisting}
Pokrenimo sljedeći playbook.
\begin{lstlisting}
---
- hosts: 127.0.0.1
  vars:
  ...
  tasks:
    - name: Jinja2 Example
      template:
        ...
        dest: /tmp/parsed.txt
\end{lstlisting}
Ako pokrenemo playbook i provjerimo na host mašini da li je ispravno parsiran predložak dobit ćemo prikaz kao na slici.
\begin{figure}[H]
\centering
 \includegraphics[scale=0.61]{slike/jinja.png}
 \caption{Korištenje jinja2 predložaka.}
 \label{ref_slikajinja}
\end{figure}
\end{document}