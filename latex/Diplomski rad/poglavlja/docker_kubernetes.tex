...
Kao što je spomenuto u prethodnom poglavlju tehnologije koje nam omogućavaju implementaciju mikroservisne arhitekture će se detaljnije obraditi u ovom poglavlju. Čitavo poglavlje je posvećeno ovim tehnologijama jer predstavljaju \textbf{eng. backbone} mikroservisne arhitekture. Da bi se upoznali s konceptom kontejnera moramo spomenuti par stvari o linux kernel-u.
\\\\
\textbf{Namespace} je abstrakcija na nivou kernela, oko globalnih sistemskih resursa koja procesima unutar namespace-a daje privid da imaju svoju izoliranu instancu globalnih resursa.
\\\\
\textbf{Namespace isolation} je koncept koji se koristi u Linux operativnom sistemu da izoliramo procese korištenjem namespace-a. Postoje različiti namespace tipovi dostupni na linux platformi, a ovdje su navedeni samo oni usko vezani za koncept kontejnera:
\begin{itemize}
...
\\\\
U suštini kontejneri, za razliku od virtualnih mašina, izvršavaju virtualizaciju na nivou operativnog sistema. \textbf{Docker} predstavlja jednu od mnogih tehnologija koji implementiraju navedene koncepte. Postoje i druge tehnologije kao što su CoreOS, containerd, lxc, itd. 
\\\\
\section{Docker}
Kontejner na višem apstrakcijskom nivou predstavlja standardnu jedinicu software-a koja pakuje izvorni kod sa svojim neophodnim zavisnostima i omogućava aplikacijama da se pokreću veoma brzo na različitim okruženjima (portabilnost). Jedna od platformi koja nam omogućava ovaj proces naziva se \textbf{Docker}. Da bi definisali sadržaj nekog kontejnera potrebno je definisati tekstualni dokument na osnovu kojeg docker izgradi \textbf{sliku kontejnera} (engl. docker image). Taj tekstualni file nazvan je Dockerfile. Slika kontejnera sadrži aplikaciju i sve njene potrebne zavisnosti da bi se aplikacija mogla pokrenuti bez problema. Slika kontejnera postaje aktivan kontejner kada se pokrene na \textbf{Docker engine}. Iako je kontejner opšti pojam, dalje u ovom radu se odnosi na docker kontejner.\\\\
...
\end{lstlisting}
Sa run komandom pokrećemo naše slike. Opcija -d govori da se kontejner pokrene kao daemon (pozadinski proces), a -p vrši mapiranje porta unutar kontejnera sa portom na host operativnom sistemu. Sada je slika postala aktivna instanca kontejner. Da bi pristupili bazi podataka sa terminala host operativnog sistema potrebno je uraditi
\begin{lstlisting}
...
\\\\
Bind mounts rade na sličan način kao i volumes. Bind mount montira direktorij sa host sistema u kontejner. Bitna razlika je u tome što nemamo nivo izolacije kao sa korištenjem volumes-a jer procesi u kontejneru direktno manipuliraju file sistemom host-a. Pored toga docker nema mogućnost upravljanjem kao što ima nad
volumes. \\
...
\end{lstlisting}
Na terminalu dobijemo detaljan ispis.
\begin{lstlisting}
[
    {
        "Name": "bridge",
...
\newpage
\section{Kubernetes}
Kubernetes predstavlja sloj oko kontejnera koji podiže nivo apstrakcije i proširuje mogućnosti pri menadžmentu i orkestraciji kontejnera. Kubernetes je također open source i dio je Cloud Native Computing Foundation. Originalno je dizajniran unutar Google-a međutim kasnije je prerastao u open source program kao dio CNCF-a. Kubernetes je primarno dizajniran da olakša orkestraciju i menadžment kontejnera na velikoj skali. Postao je defacto standard kod svih velikih javnih cloud provajdera.
...
Master node održava RESTful web servise za postavljanje upita i definisanje stanja cluster-a. Pristup REST web servisu na master serveru ostvarujemo pomoću \textbf{kubectl} skripte koja nam omogućava da stanje koje smo definisali primjenimo na cluster sa terminala. Pored manuelne intervencije putem \textbf{kubectl} na Master node-u također su aktivne sljedeće komponente:
\begin{itemize}
...
\end{itemize}
Kubernetes zadano koristi kubenet driver koji je veoma jednostavan. Networking unutar kubernetes se može objasniti drugačije u ovisnosti koji se network driver koristi. U ovom slučaju je korišten kubenet. Na slici ispod dat je prikaz komunikacije između pod-ova unutar jednog node-a.
\begin{figure}[H]
\centering
 ...
 \label{ref_slika2}
\end{figure}
Ako analiziramo sliku možemo primjetiti da postoji velika sličnost sa docker bridge tipom mreže. Razlika se ogleda u tome što pod sadrži više kontejnera koji dijele mrežni namespace. Na taj način omogućena je komunikacija između kontejnera u jednom pod-u korištenjem loopback adrese (localhost). Posljedica toga je da u jednom pod-u ne možemo imati preklapanja u korištenim portovima na različitim kontejnerima. \\\\Na sljedećoj slici je data topologija mreže gdje je omogućena inter-node komunikacija.
\begin{figure}[H]
\centering
 \includegraphics[scale=0.70]{slike/kubernetes_n2n_network.pdf}
 \caption{Topologija kubernetes mreže. Primjer inter node komunikacije.}
 \label{ref_slika2}
\end{figure}
Objasnit ćemo za primjer komunikaciju od Pod1 do Pod4. Kako svaki pod unutar jednog node-a posjeduje jedinstvenu IP adresu ARP rezolucija neće pronaći odgovarajuću IP adresu unutar datog node-a (gdje se nalazi Pod1). Paket onda izlazi van node-a i rutira se prema definisanim pravilima do node-a koji sadrži destinacijsku IP adresu paketa. Kubenet ne podržava inter node komuminkaciju međutim driveri kao što su Calico podržavaju komunikaciju između pod-ova koji se nalaze na različitim node-ovima. \\\\
Kod inter node komunikacije, gdje destinacijska IP adresa nekog pod-a izlazi van domena klastera, koristi se overlay mreža. Najčešće driveri implementirani od treće strane koriste SNAT mapiranje pri izlasku paketa iz klastera. Mapira se IP adresa izvornog pod-a na IP adresu node-a na kojem se nalazi dati pod. Na ovaj način omogućeno je rutiranje sve do destinacijske IP adrese. Kubenet podržava samo mrežu unutar jednog klastera. Da bi koristili overlay mrežu potrebno je koristiti network drivere kao što su: C0alico, Flannel, ACI (Cisco), kao i mnogi drugi. 
\subsubsection{Servisi}
Kako se u toku vremena pod-ovi mogu rađati i umirati, dolazi do potrebe povezivanja pod-ova sa nekom fiksnom domenom. Svaki objekat u kubernetes se može označiti pomoću \textbf{labela}. Na osnovu labela moguće je povezati pod objekat sa objektom servis. Na taj način osiguravamo da iako dolazi do internih promjena unutar node-a,  pristup s vana određenom podu ostaje nepromijenjen. Servisi dakle predstavljaju abstrakciju pristupa pod-ovima vanjskim korisnicima. \\\\
...
Definicija i konfiguracije kubernetes klastera se zasniva na objektima. Objekte definišemo korištenjem yaml zapisa. Slično kao kod ansible-a. Svi pojmovi navedeni u poglavlju Kubernetes: pod, servis, deployment, itd... Mogu se definisati, konfigurisati i primjeniti na klaster korištenjem kubernetes objekta. 
\subsubsection{Pods}
...
\end{figure}
\subsubsection{Servisi}
Definicija objekta tipa servis omogućava nam da definišemo stalan pristup pod-ovima koji mogu mijenjat svoju IP adresu u toku vremena. Ispod je data definicija objekta tipa NodePort servis.
\begin{lstlisting}
apiVersion: v1
apiVersion: v1
kind: Service
...
spec:
  ...
  ports:
    - protocol: TCP
      ...
      nodePort: 30080
\end{lstlisting}
Servis se povezuje sa svakim pod-om koji sadrži labelu \textbf{app: pod1-app} u svjoj definiciji. Kako se u toku vremena labela neće promijeniti na ovaj način pristup pod-ovim sa labelom pod1-app će ostati stalan. Da bi pristupili pod-ovima preko servisa dovoljno je uputiti zatjev prema \textbf{1} i DNS rezolucija unutar kubernetesa će izvršti rezoluciju u IP adresu i to na osnovu Round-robin algoritma.
\subsubsection{Namespace}
Još jedan bitan aspekt Kubernetes-a je namespace. Namespace nam dozvoljava da grupišemo Kubernetes objekte u logičke grupe i na taj način vršimo razdavajanje istih. Namespace nam omogućava kreiranje virtualnih cluster-a.\\\\
Definicija kubernetes objekta tipa namespace data je ispod.
\begin{lstlisting}
apiVersion: v1
kind: Namespace
metadata:
  name: custom_ns_name
\end{lstlisting}
Da bi kreirali namespace potrebno je aplicirati ga sa kubectl komandom.
\begin{lstlisting}
kubectl apply -f namespace.yaml
\end{lstlisting}
Kreairanje drugih objekata u nekom namespace-u korištenjem kubectl omogućeno je proslijeđivanjem dodatnog paramtera.
\begin{lstlisting}
kubectl apply -f object.yaml --namespace=custom_ns_name
\end{lstlisting}
Na ovaj način object.yaml će biti apliciran samo u \textbf{custom\_ns\_name} namespace-u.
\newpage
\subsubsection{NetworkPolicy}
NetworkPolicy predstavlja veoma bitnu vrstu kubernetes objekta. NetworkPolicy omogućava kontrolu prometa paketa na layer-ima 3 i 4. To znači da na nivou aplikacije možemo definisati pravila komunikacije pod-ova sa entitetima na mreži. Također moguće je razdvojiti kontrolu paketa za egres i ingress promet.
\\\\
NetworkPolicy omogućava kontrolu paketa na osnovu:
\begin{itemize}
    \item Namespace iz kojeg paket dolazi/odlazi
    \item Pod-a iz kojeg paket dolazi/odlazi
    \item IP blok adresa iz kojeg dolazi/odlazi
\end{itemize}
Primjer NetworkPolicy objekta je dat ispod.
\begin{lstlisting}
...
spec:
  ...
  - to:
    ...
    - protocol: TCP
      port: 5978
\end{lstlisting}
U primjeru je definisan NetworkPolicy za egress i ingress tip prometa. Također su korištena sva tri načina definisanja kontrole, navedena u listi iznad.
\end{document}
